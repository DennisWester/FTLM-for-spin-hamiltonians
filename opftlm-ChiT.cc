				 ...
				 const string inputname, const long samen, const double exec_time, const double cpu_time)
{
	...
	string infodat = "FTLM-Informationen-Chi_";
	infodat += magtext;
...
long double magnetisierung(const double *allEW, const double *allsquarednr, const complex<double> *SerwBrichtung, const int lanczosSteps, const int random_vectors, const int richtungen,
						   const unsigned long hilbertraumDimension, const double temperatur, double *orientierungsmittel)
{
...
double susceptibility(const double *allEW, const double *allsquarednr, const complex<double> *SerwBrichtung, const int lanczosSteps, const int random_vectors, const int richtungen,
					  const unsigned long hilbertraumDimension, const double temperatur, const double b_Feld, double *orientierungsmittel)
{
	double suscept = (double) magnetisierung(allEW, allsquarednr, SerwBrichtung, lanczosSteps, random_vectors, richtungen, hilbertraumDimension, temperatur, orientierungsmittel);
	suscept *= temperatur / b_Feld;
	suscept *= 0.558494; //Factor for the conversion of mu_B to erg*K/G²*mol
	return suscept;
}
...
int main(int argc, char *argv[]) 
{
	...
	for (auto feldcounter : vecbfeld)
	{
		...
		{	
			long double chi = susceptibility(allEW.get(), allsquarednr.get(), SerwBrichtung.get(), lanczosSteps, random_vectors, richtungen, hilbertraumDimension, t, feldstaerke, orientierungsmittel);
			outmag << setprecision(11);
			outmag << chi << "  " << t << "  " << feldstaerke << endl;
		}
		outdat.close();
	}
